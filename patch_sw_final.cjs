...
// Above replaces the crash handling loop logic.
...
// Above replaces the tab activated logic
...
content = content.replace(
  ...
  'const activeAgent = new AgentLoop({ // REPLACED'
);
...
content = content.replace(
  ...
  'if (!activeAgent || activeAgent.state === AgentState.IDLE) return { status: "OK", state: AgentState.IDLE };' // keep as is, it uses the local variable
);
