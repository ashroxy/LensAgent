...
export async function loadSettings() {
  const saved = await localGet("userSettings", {});
  const finalSettings = { ...DEFAULT_SETTINGS }; for (const k in saved) { if (saved[k] !== "" && saved[k] !== null && saved[k] !== undefined) finalSettings[k] = saved[k]; } return finalSettings;
}
