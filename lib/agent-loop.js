/**
 * agent-loop.js â€” Perception â†’ Decision â†’ Action Orchestrator
 * =============================================================
...
export class AgentLoop {
  ...
  async start(goal) {
    ...
    await storage.appendLog(`Agent started â€” Goal: "${goal}"`, "action");
  ...
  pause() {
    ...
    this.isBusy = false;
    this._broadcastStatus();
    this._log("Agent paused â€” debugger detached. Close DevTools and restart.", "warning");
  }
  ...
  async _processCycle(frame, cycleStart) {
    ...
    // â”€â”€â”€ Stage 1: Perception (FAIL-CLOSED) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    // SECURITY INVARIANT: If ANY step in the redaction pipeline fails,
    // the raw frame is DROPPED â€” never forwarded to the network.
    // This guarantees zero data egress even under crash conditions.
    const percStart = Date.now();
    let perception;
    let piiBoxes = [];

    // 1a. PII scanning â€” fail-closed
    try {
      ...
      return; // DROP THE FRAME â€” never send raw data
    }

    // 1b. Offscreen perception (redaction) â€” fail-closed
    try {
    ...
    // 1c. Validate redacted output exists â€” fail-closed
    ...
    if (this.captureEngine.epoch !== epoch) {
      this._log("Page navigated during perception â€” skipping stale result", "info");
      return;
    }

    // â”€â”€â”€ Emit Audit Update â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if (this.enableAudit) {
    ...
    // â”€â”€â”€ Stage 2: Server Decision â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    if (this.captureEngine.epoch !== epoch) {
      this._log("Page navigated during server call â€” dropping stale action", "info");
      return;
    }

    // â”€â”€â”€ Stage 3: Execute Actions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    for (let i = 0; i < actions.length; i++) {
      ...
      if (action.type === ActionType.VAULT_FILL) { action.type = ActionType.TYPE; }`n`n        // ASK_USER
      ...
      this._log(`${icon} Step ${this.stepCount}${actions.length > 1 ? `.${i + 1}` : ""}: ${result.action} â€” ${result.detail}${result.retries > 0 ? ` (${result.retries} retries)` : ""}`, result.success ? "action" : "error");
    ...
    // â”€â”€â”€ Post-Cycle â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  async _requestServerAction(perception, frame) {
    ...
    // â”€â”€ Privacy Gate: Strip PII from AX tree before it reaches the LLM server â”€â”€
    ...
    const payload = {
      ...
      // Stream 3: Accessibility Tree â€” PII sanitized by Member 3's engine
      a11y_tree:      a11yTree,
