...
export class AgentLoop {
  ...
  async _requestServerAction(perception, frame) {
    ...
    const [rawA11yTree, domSnapshot] = await Promise.all([
    ...
    // Member 3's AccessibilitySanitizer replaces real PII (Aadhaar, PAN, phone)
    // in node name/value/description fields with [REDACTED_*] tokens.
    // This is the LAST line of defence before data leaves the browser.
    ...
    // Build backend-compatible elements
    const backendElements = await this._buildBackendCompatibleElements();
    ...
    const payload = {
      ...
      available_keys: this.vault ? this.vault.getAvailableKeyNames() : [],
      execution_results: this._lastExecutionResults
    };

    const { PrivacyEngine } = await import('../privacy_engine.js');
    const privacyEngine = new PrivacyEngine({ enableStrictZeroLeakage: true });
    try {
      privacyEngine.validatePayload(payload);
    } catch (error) {
  ...
  /**
   * Builds backend-compatible element array matching Member 4's expected schema
   * Extracts ALL elements from the page with full metadata for visibility/enabled checks
   * @returns {Promise<Array>} Array of element objects with backend-expected schema
   */
  async _buildBackendCompatibleElements() {
    try {
      // Get ALL elements from the DOM (not just preset selectors)
      const allElements = await this.actionExecutor._getAllElementsDeep(document.body);
      ...
      return allElements
        .map(el => {
          ...
          const elementId = el.id || `auto_${Date.now()}_${Math.floor(Math.random() * 10000)}`;

          // Determine role from element attributes
          const role = this._getRoleFromElement(el);

          // Get associated label (for form elements)
          const label = this._getAssociatedLabel(el) ||
          ...
          return {
            ...
            visible: this._isElementVisible(el, rect),
            enabled: !el.disabled,
            checked: el.type === 'checkbox' ? el.checked : undefined,
            selected: el.tagName === 'SELECT' ? el.selected : undefined
          };
        })
        .filter(Boolean); // Remove null elements
    } catch (err) {
      console.warn("[AgentLoop] Failed to build backend-compatible elements:", err.message);
      // Fallback to existing DOM snapshot if new method fails
      return this.actionExecutor.getDOMSnapshot();
    }
  }

  /**
   * Determines the ARIA role of an element
   * @param {Element} el - DOM element
   * @returns {string} Role string
   */
  _getRoleFromElement(el) {
    ...
    switch (tag) {
      case 'input':
        switch (el.type) {
          ...
          case 'text': case 'password': case 'email': case 'tel': case 'url': case 'search':
          case 'date': case 'datetime': case 'datetime-local': case 'month': case 'week':
          case 'time': case 'number': case 'range': case 'color': return 'textbox';
      ...
      default: return el.getAttribute('role')?.toLowerCase() || 'generic';
    }
  }

  /**
   * Gets the associated label for form elements
   * @param {Element} el - Form element
   * @returns {string|null} Label text or null
   */
  _getAssociatedLabel(el) {
    ...
    // Check for associated label element via htmlFor
    const id = el.id;
    if (id) {
   ...
   * @param {Element} el - DOM element
   * @param {DOMRectReadOnly} rect - Element's bounding rectangle
   * @returns {boolean} True if visible
   */
  _isElementVisible(el, rect) {
