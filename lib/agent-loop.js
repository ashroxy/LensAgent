...
export class AgentLoop {
  ...
  async _processCycle(frame, cycleStart) {
    ...
    let perception;
    try {
      perception = await this._requestPerception(frame);
    } catch (err) {
      this._log(`Perception failed: ${err.message}`, "warning");
    ...
    const actions = decision.actions || (decision.action_plan ? [decision.action_plan] : []);
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        this.offscreenPort.postMessage(
          { type: BG_PROCESS_FRAME, correlationId: id, rawBase64: frame.base64, buffer: frame.buffer },
          [frame.buffer]
        );
      } catch (err) {
        this._pendingPerception.delete(id);
  ...
  async _requestServerAction(perception, frame) {
    ...
    const [a11yTree, domSnapshot] = await Promise.all([
