...
export class AgentLoop {
  ...
  async _processCycle(frame, cycleStart) {
    ...
    try {
      ...
      await this._showOverlayError(err.message || "Server Error");
  ...
  async _requestPerception(frame, piiBoxes = [], viewportWidth = null) {
    const startTime = Date.now();
    try {
      const response = await chrome.runtime.sendMessage({
        type: "PROCESS_SCREENSHOT",
        dataUrl: frame.base64,
        domBoxes: piiBoxes
      });
      ...
      return {
        redactedImageBase64: response.sanitizedBase64,
        elements: [], // Replaced by DOM elements later
        redactedRegions: Array(response.detectionCount).fill({}),
        latencyMs: response.latencyMs
      };
    } catch (err) {
      this._log(`[Perception Error] ${err.message}`, "error");
      throw err;
    }
  }

  _setupPerceptionListener() {
    // No longer needed, using standard async sendMessage
  }
  ...
  async _showOverlay() {
    try {
      await this.actionExecutor._cdp("Runtime.evaluate", {
        expression: `
          (function() {
            if (document.getElementById('lens-agent-analyzing-overlay')) return;
            const overlay = document.createElement('div');
            overlay.id = 'lens-agent-analyzing-overlay';
            ...
            spinner.id = 'lens-agent-spinner';
            Object.assign(spinner.style, {
              width: '50px', height: '50px', border: '5px solid rgba(0, 255, 0, 0.2)',
              borderTopColor: '#00ff00', borderRadius: '50%', animation: 'lens-agent-spin 1s linear infinite'
            });
            
            const text = document.createElement('div');
            text.id = 'lens-agent-text';
            text.textContent = 'Analyzing...';
            Object.assign(text.style, {
              marginTop: '20px', color: '#00ff00', fontSize: '20px', fontWeight: 'bold',
              fontFamily: 'system-ui, sans-serif', textShadow: '0 2px 4px rgba(0,0,0,0.5)',
              letterSpacing: '1px'
            });
            
            const style = document.createElement('style');
            style.textContent = '@keyframes lens-agent-spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }';
  ...
  async _removeOverlay() {
    try {
      await this.actionExecutor._cdp("Runtime.evaluate", {
        expression: `
          (function() {
            const overlay = document.getElementById('lens-agent-analyzing-overlay');
  ...
  async _showOverlayError(errorMsg) {
    try {
      await this.actionExecutor._cdp("Runtime.evaluate", {
        expression: `
          (function() {
            let overlay = document.getElementById('lens-agent-analyzing-overlay');
            if (!overlay) {
            ...
            const spinner = document.getElementById('lens-agent-spinner');
            if (spinner) spinner.remove();
            
            let text = document.getElementById('lens-agent-text');
            if (!text) {
            ...
            Object.assign(text.style, {
              marginTop: '20px', color: '#ff4444', fontSize: '20px', fontWeight: 'bold',
              fontFamily: 'system-ui, sans-serif', textShadow: '0 2px 4px rgba(0,0,0,0.5)'
            });
            
            let detail = document.getElementById('lens-agent-detail');
            if (!detail) {
              detail = document.createElement('div');
              detail.id = 'lens-agent-detail';
              overlay.appendChild(detail);
            }
            detail.textContent = ${JSON.stringify(errorMsg || "Server error")};
            Object.assign(detail.style, {
              marginTop: '10px', color: '#ffaaaa', fontSize: '14px', fontFamily: 'monospace',
