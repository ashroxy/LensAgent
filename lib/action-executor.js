                ...
                if (tag === 'input' || tag === 'textarea' || tag === 'select' || el.isContentEditable) {
                  ...
                  if (val.length > 0) {
                    for (const [cat, regex] of Object.entries(patterns)) {
                      regex.lastIndex = 0;
                      if (regex.test(val)) {
                        sensitiveItems.push({ id: 'SEC_' + cat.toUpperCase() + '_' + (counter++), type: 'INPUT_FIELD', category: cat.toUpperCase(), boundingBox: { x: Math.round(rect.left), y: Math.round(rect.top), width: Math.round(rect.width), height: Math.round(rect.height) }, redactionLabel: '[REDACTED_' + cat.toUpperCase() + '_#' + (counter-1) + ']' });
                        return;
                      }
                    }
                  }
                  ...
                  while ((match = regex.exec(text)) !== null) {
                    try {
                      ...
                      if (rect.width > 0 && rect.height > 0) {
                        sensitiveItems.push({ id: 'PII_' + cat.toUpperCase() + '_' + (counter++), type: 'TEXT_PII', category: cat.toUpperCase(), boundingBox: { x: Math.round(rect.left), y: Math.round(rect.top), width: Math.round(rect.width), height: Math.round(rect.height) }, redactionLabel: '[REDACTED_' + cat.toUpperCase() + '_#' + (counter-1) + ']' });
                      }
                    } catch (e) {}
                  }
                }
