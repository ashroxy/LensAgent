...
if (sensitiveKeywords.test(attrContext) || dataPii) {
  ...
  return;
              ...
              return { boxes: sensitiveItems, viewportWidth: window.innerWidth };
} catch (e) { return { boxes: [], viewportWidth: 1280 }; }
})()
,
returnByValue: true,
});
const val = res?.result?.value || { boxes: [], viewportWidth: 1280 };
this._lastViewportWidth = val.viewportWidth;
return val.boxes;
} catch (err) {
