...
export class CaptureEngine {
  ...
  /**
   * Capture an immediate on-demand screenshot frame (for static pages).
   * @returns {Promise<Object>}
   */
  async captureFrame() {
    if (!this.isAttached) throw new Error("Debugger not attached.");
    const res = await this._cdp("Page.captureScreenshot", {
      format: "jpeg",
      quality: this.quality,
      fromSurface: true
    });
    if (!res || !res.data) throw new Error("Capture screenshot returned empty data");
    const data = res.data;
