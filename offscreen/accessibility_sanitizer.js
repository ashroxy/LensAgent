/**
 * ISRO PS 171 - Accessibility Tree Sanitizer
 * Module: accessibility_sanitizer.js
 *
 * Sanitizes the accessibility tree before it is sent to the LLM,
 * replacing PII in node name/value/description fields with [REDACTED_*] tokens.
 * Handles password fields with a total wipe.
 */
  ...
  sanitizeTree(axTree) {
    ...
    const sanitizeNode = (node) => {
      ...
      if (isProtected) {
        ...
        if (typeof cleanNode.name === 'string')        cleanNode.name        = this._maskPIIString(cleanNode.name);
        if (typeof cleanNode.value === 'string')       cleanNode.value       = this._maskPIIString(cleanNode.value);
        if (typeof cleanNode.description === 'string') cleanNode.description = this._maskPIIString(cleanNode.description);
      }
